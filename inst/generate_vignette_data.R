...
rep1_clusters[rp_names,"x_centroid"] = cells[match(rp_names,row.names(cells)),
                                             "x_centroid"]
rep1_clusters[rp_names,"y_centroid"] = cells[match(rp_names,row.names(cells)),
                                             "y_centroid"]
...
rep1_clusters$rep="rep1"

rep1_clusters = rep1_clusters[rep1_clusters$anno != "Unlabeled", ]


keep_cells=rep1_clusters[(rep1_clusters$x_centroid>=200 &
                              rep1_clusters$x_centroid<=500) &
                             (rep1_clusters$y_centroid>=200 &
                                  rep1_clusters$y_centroid<=1000),
                         "cells"]
...
rep1_sub = list(cm=rep1$cm[setdiff(row.names(rep1$cm),rm_genes),keep_cells],
                trans_info=rep1$trans_info[rep1$trans_info$cell_id %in% keep_cells &
...
rep2_clusters[rp_names,"x_centroid"] = cells[match(rp_names,row.names(cells)),"x_centroid"]
rep2_clusters[rp_names,"y_centroid"] = cells[match(rp_names,row.names(cells)),"y_centroid"]

rep2_clusters$cluster=factor(rep2_clusters$cluster,
                             levels=paste("c",1:12, sep=""))
rep2_clusters$cells =row.names(rep2_clusters)
rep2_clusters$rep="rep2"
rep2_clusters = rep2_clusters[rep2_clusters$anno != "Unlabeled", ]


keep_cells2=rep2_clusters[(rep2_clusters$x_centroid>=200 &
                               rep2_clusters$x_centroid<=500) &
                              (rep2_clusters$y_centroid>=200 &
                                   rep2_clusters$y_centroid<=1000) ,
                          "cells"]
...
rep2_sub = list(cm=rep2$cm[setdiff(row.names(rep2$cm),rm_genes),keep_cells2],
                trans_info=rep2$trans_info[rep2$trans_info$cell_id %in% keep_cells2 &
