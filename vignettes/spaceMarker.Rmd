...
### 10x Xenium data
For loading the 10x Xenium data,  the help function`get_xenium_data()` can be 
used. The cosntructed xenium_data can be used in function `get_vectors()`.
...
xenium_data = get_xenium_data(path, mtx_name="cell_feature_matrix",
...
If you are working with Merscope or CosMx data, the following code can be 
used for constructing the required data structure. 
...
### Visualise top3 marker genes for each cluster detected by linear modelling approach
