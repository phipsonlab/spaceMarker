---
title: "koalasso example"
date: "`r format(Sys.time(), '%d %b %Y')`"
vignette: >
    %\VignetteIndexEntry{koalasso example}
...
# cluster, x_centroid, y_centroid and rep
...
w_x =  c(min(floor(min(data$trans_info$x_location)),
            floor(min(clusters_info$x_centroid))), 
        max(ceiling(max(data$trans_info$x_location)),
            ceiling(max(clusters_info$x_centroid))))
w_y =  c(min(floor(min(data$trans_info$y_location)),
            floor(min(clusters_info$y_centroid))), 
        max(ceiling(max(data$trans_info$y_location)),
            ceiling(max(clusters_info$y_centroid))))
...
# cluster, x_centroid, y_centroids, rep and cell_id
...
p1<-ggplot(data = rep1_clusters,
        aes(x = x_centroid, y = y_centroid, color=cluster))+
...
w_x =  c(min(floor(min(rep1_sub$trans_info$x_location)),
        floor(min(rep1_clusters$x_centroid))), 
        max(ceiling(max(rep1_sub$trans_info$x_location)),
        ceiling(max(rep1_clusters$x_centroid))))
w_y =  c(min(floor(min(rep1_sub$trans_info$y_location)),
        floor(min(rep1_clusters$y_centroid))), 
        max(ceiling(max(rep1_sub$trans_info$y_location)),
        ceiling(max(rep1_clusters$y_centroid))))

loc_mt = as.data.frame(rep1_sub$trans_info[rep1_sub$trans_info$feature_name %in% 
          c("ERBB2","EPCAM"),
          c("x_location","y_location","feature_name")]%>%distinct())
...
loc_mt =as.data.frame(rep1_clusters[rep1_clusters$cluster %in% c("c1","c2"),
            c("x_centroid","y_centroid","cluster")])
...
w_x =  c(min(floor(min(rep1_sub$trans_info$x_location)),
            floor(min(rep2_sub$trans_info$x_location)),
            floor(min(rep_clusters$x_centroid))), 
        max(ceiling(max(rep1_sub$trans_info$x_location)),
            ceiling(max(rep2_sub$trans_info$x_location)),
            ceiling(max(rep_clusters$x_centroid))))
w_y =  c(min(floor(min(rep1_sub$trans_info$y_location)),
            floor(min(rep2_sub$trans_info$y_location)),
            floor(min(rep_clusters$y_centroid))), 
        max(ceiling(max(rep1_sub$trans_info$y_location)),
            ceiling(max(rep2_sub$trans_info$y_location)),
            ceiling(max(rep_clusters$y_centroid))))
...
for (cl in all_celltypes){
    ...
    if (length(inters > 0)){
        ...
        p1<- ggplot(data = vis_r1,
                    aes(x = x_location, y = y_location))+ 
                    geom_point(size=0.01,color="black")+
                    facet_wrap(rep~text_label,ncol=10, scales="free")+
                    scale_y_reverse()+
                    guides(fill = guide_colorbar(height= unit(5, "cm")))+
                    theme(strip.text = element_text(size = rel(0.7)), 
                    axis.line=element_blank(),
